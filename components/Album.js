...
export function Album({ title, imageUrl, date }) {
  return (
    <Link href={'albums/' + title.toLowerCase().replace(' ', '-')} className="relative w-full aspect-3/4 max-w-[28rem] items-end justify-center overflow-hidden text-center rounded-2xl hover:scale-105 active:scale-100 duration-[250ms] cursor-pointer tracking-wider leading-loose shadow-lg">
      <Image
        loader={imageLoader}
        alt="ALT TEXT"
        src={imageUrl}
